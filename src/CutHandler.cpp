...
CutHandler::CutHandler(const std::string &edename, const std::string &xxname) :
    edefile(nullptr), xxfile(nullptr), edecut(nullptr), xxcut(nullptr), validFlag(false)
{
...
CutHandler::~CutHandler()
{
    if (edefile != nullptr && edefile->IsOpen())
        edefile->Close();
    if (xxfile != nullptr && xxfile->IsOpen())
        xxfile->Close();
}

void CutHandler::SetCuts(const std::string& edename, const std::string& xxname)
{
